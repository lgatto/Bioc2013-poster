  ...
  \begin{columns}
    \begin{column}{.48\textwidth}
     ...
     \begin{block}{Introduction}
       ...
       \textbf{raw MS data} manipulation, labeled and label-free \textbf{quantitation} and peptide \textbf{identification}.       
     \end{block}
...
\Rpackage{xcms}, \Rpackage{MSnbase}, \Rpackage{RMassBank} and \Rpackage{TargetSearch}.
     ...
     \begin{block}{Labelled quantitation}
       ...
       directly processed, plotted, quantified and normalised with the \Rpackage{MSnbase} software. 
       \begin{figure}[h]
<<plotspectrum, dev = 'pdf', fig.width = 12, fig.height = 5>>=
...
<<msnbase, cache=TRUE, warnings = FALSE>>=
set <- quantify(exp, method = "trap", reporters = iTRAQ4, verbose = FALSE, parallel = TRUE)
head(exprs(set), n = 3)
@        

     \textbf{Challenges} Altough labelled MS$^2$ quantitation is well supported with \Rpackage{MSnbase} 
     and \Rpackage{isobar}, metabolic labelling techniques like N$^{15}$ or SILAC still need to be developed. 
