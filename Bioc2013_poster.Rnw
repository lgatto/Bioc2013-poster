  ...
  \begin{columns}
    \begin{column}{.48\textwidth}
     ...
     \begin{block}{Working with raw data}
       ...
       of closed, binary vendor-specific formats. One of the main projects that implement parsers for the XML-based 
...
\Rpackage{xcms}, \Rpackage{MSnbase}, \Rpackage{RMassBank} and \Rpackage{TargetSearch}.
     ...
     \begin{block}{Labelled quantitation}
       \justifying 
       The same raw data file can be imported in a convenient higher level container and 
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \label{fig:msnbase} 
  \caption{MS$^2$ spectrum of an iTRAQ 4-plex experiment showing the 4 isobaric reporter ions, 
    as produced by \Rfunction{plot} above (left). Peptides of interesting from a spiked-in experiment 
    (top right) and distribution of the \textit{m/z} differences of all MS$^2$ spectra from the same 
    experiment, use as a peptide-spectrum matching quality assessment (bottom right).}
\end{figure}

     \textbf{Challenges} Although labelled MS$^2$ quantitation is well supported with \Rpackage{MSnbase} 
     and \Rpackage{isobar}, metabolic labelling techniques like $^{15}$N or SILAC still need to be developed. 
    ...
    \begin{column}{.48\textwidth}
      ...
      \begin{block}{Label-free quantitation}
        \justifying 
        Label-free quantitation is available in the \Rpackage{xcms} \cite{xcms} and \Rpackage{MALDIquant} \cite{maldiquant} packages. 
        ...
        \begin{figure}[h]
          ...
          \caption{Illustration of the \Rpackage{MALDIquant} pipeline:
            ...
            variance-stabilized, smoothed, baseline-corrected spectrum with detected peaks (B);
            fitted warping function for peak alignment (C); 
            four unaligned peaks (D);
            four aligned peaks (E); 
            merged spectrum with discovered and labeled peaks (F).}
        \end{figure}
        
        A complete pipeline for MS$^e$ data independent acquisition, including support for ion mobility separation 
        is available in the \Rpackage{synapter} package \cite{synapter} that, among other, transfers identification between acquisitions 
        to substantially reduce missing values. 
        
        \bigskip        
        \textbf{Challenges} Application and benchmarking of label-free pipeline on popular Thermo Orbitrap instruments.        
      \end{block}
      %%\vfill
      \begin{block}{Peptide identification}
        \justifying 
        The recently released \Rpackage{rTANDEM} package encapsulates the \texttt{X!Tandem} \cite{tandem}
        search engine in \R and uses the same XML-based parameter files as the native application 
...
## pr xmlres <- tandem(param)
res <- GetResultsFromXML(xmlres)
proteins <- GetProteins(res) ## data.table objects
peptides <- GetPeptides(res) 
@       
        A complete pipeline with support for identification is welcome at the cost of additional 
        development and maintenance time for developers. With support for \texttt{mzIdentML} files, 
        it will become possible to import identification data from most search engines, this 
        facilitating the integration of \R based pipelines with existing tools.        
        
        \bigskip        
        \textbf{Challenges} Better integration of identification and raw/quantitation data infrastructures.
      ...
      \begin{block}{Conclusions and perspectives}
       ...
       The flexibility of the \R environment and the breath of available packages is sometimes 
       daunting for newcomers and introductory points of entry are welcome.  
