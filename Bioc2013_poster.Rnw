...
\newcommand{\challenge}[1]{
       \begin{tcolorbox}[notitle,boxrule=0pt,colback=blue!10,colframe=blue!10]
         \textbf{Challenges}~{#1}
       \end{tcolorbox}
}
...
\end{figure}
    ...
    \begin{column}{.48\textwidth}
      ...
      \begin{block}{Peptide identification}
        \justifying 
        The recently released \Rpackage{rTANDEM} package encapsulates the \texttt{X!Tandem} \cite{tandem} search engine in \texttt{R}.
        \begin{Leftbar}
